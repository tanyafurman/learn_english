...
/**
 * Класс ApplicationFrame создает основной фрейм, в котором содержиьбся главное меню, то есть: 
 * Words (таблицы с словами для изучения, словарь и уже известные слова), Test, Status (статистика результатов пользователя).<br>
 * <br>
 * Данный класс имеет две кнопки и подпанель UI, в которой устанавливается расположение названия таблицы и скролл.<br>
 * <br>
 *Класс ApplicationFrame содержит такие переменные, как:<br>
 *1) меню для пльзователя.<br>
 *2) таблицы, которые содержат все слова.<br>
 *<br>
 *<br>
 *Данный класс выполняет такие функции, как:<br>
 *1) создание фрейма, где будут изображены таблицы со словами.<br>
 *2) создание фрейма, где пользователь проходит тест.<br>
 *3) создание фрейма, где пользователь сможет посмотреть результаты прохождения теста.<br>
 *4) создание основного меню (Words, Test, Status).<br>
 *<br>
 *<br>
 *Класс ApplicationFrame имеет вложенные классы такие, как:<br>
 *	<br>
 *Класс CreateUserAction предназначен для создания пользователя. 
 *Этот класс считывает имя, фамилию, логин и пароль, которые вводит пользователь, 
 *после чего запоминает все данные.<br>
 *<br>
 *Класс LoginAction предназначен для пользователя, который хочет войти в программу с помощью ввода своего 
 *логина и пароля. Этот класс проверяет на правильность и существование введенного пользователем логина и 
 *пароля. Если есть соответсвие, то пользователь имеет досткп к тесту, словам и словарю, иначе программа выдает ошибку.<br>
 *<br>
 *Класс UpdateUserSettings предназначен для того, чтобы пользователь мог изменить настрой программы, а именно
 *количество слов, которые будет изучать пользователь и количество повторений правильного ответа на одно слово.<br>
 */
